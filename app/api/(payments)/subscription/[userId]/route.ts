...
export async function GET(
  ...
  try {
    ...
    return new NextResponse(JSON.stringify(subscriptionstatus), {
      status: 200,
    });
  } catch (error: any) {
...
async function checksubscriptionstatus(userId: string) {
    ...
    if (checkfreesubscriptionplan?.payment === SubscriptionPlan.FREE) {
      ...
      if (currentDate.getTime() < freePlanExpiryDate.getTime()) {
        return {
          ...
          expiringdate: freePlanExpiryDate.toLocaleDateString(),
        };
      }
    }
  ...
  const hasCurrentSubscriptionFinished =
    new Date(recentsubscription.nextpaymentdate).getTime() <
    new Date().getTime();
  ...
  return {
    issubscriptionfinished: hasCurrentSubscriptionFinished,
    expiringdate: recentsubscription.nextpaymentdate.toLocaleDateString(),
  };
}
